package com.sesac.finewiki.util;
...
public class UploadFileUtils {
	...
	private static String makeIcon(String uploadPath, String savedPath, String file_name) throws Exception {
		// 아이콘 파일명 = 기본 저장경로 + 날짜경로 + 구분자 + 파일명
		String iconName = uploadPath + savedPath + File.separator + file_name;
		return iconName.substring(uploadPath.length()).replace(File.separatorChar, '/');
	}

	// 파일 삭제처리 메서드
	public static void removeFile(String uploadPath, String file_name) {
		// 파일 확장자 추출
		String formatName = file_name.substring(file_name.lastIndexOf(".") + 1);
		...
		if (mediaType != null) {
			// 원본 이미지의 경로 + 파일명 추출
			// 날짜 경로 추출
			String front = file_name.substring(0, 12);
			// UUID + 파일명 추출
			String end = file_name.substring(14);
			// 원본 이미지 파일 삭제(구분자 변환)
