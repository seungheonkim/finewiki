					...
					<div class="row mb-2">
						...
						<div class="col-sm-6">
							<ol class="breadcrumb float-sm-right">
								...
								<li class="breadcrumb-item active">주식 게시판</li>
							</ol>
						</div>
						<!-- /.col -->
					</div>
									...
									<table class="table table-bordered">
										<thead>
											<tr>
												...
												<th>제목</th>
												<td><a
													href="${path}/board/stock/read${pageMaker.makeSearch(pageMaker.criteria.page)}&data_no=${stock.data_no}">
														${stock.title} </a> <span class="badge bg-teal"><i
														class="fas fa-comment"></i> + ${stock.reply_cnt}</span></td>
											...
											<c:forEach items="${stocks}" var="stock">
												<tr>
													...
													<td><a
														href="${path}/board/stock/read${pageMaker.makeSearch(pageMaker.criteria.page)}&data_no=${stock.data_no}">
															${stock.title} </a></td>
													<td>${stock.reply_cnt}</td>
													<td>${stock.mem_nick}</td>
													<td><fmt:formatDate value="${stock.regdate}"
															pattern="yyyy-MM-dd-hh-mm-ss" /></td>
													<td><span class="badge bg-success">${stock.hit}</span></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
										...
										<ul class="pagination justify-content-center m-0">
											<c:if test="${pageMaker.prev}">
												<li class="page-item"><a class="page-link"
													href="${path}/board/stock/list${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
											</c:if>
											<c:forEach begin="${pageMaker.startPage}"
												...
												<li class="page-item"
													...
													<a class="page-link"
													href="${path}/board/stock/list${pageMaker.makeQuery(idx)}">${idx}</a>
												</li>
											</c:forEach>
											<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
												<li class="page-item"><a class="page-link"
													href="${path}/board/stock/list?${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
											</c:if>
										</ul>
									</nav>
									<div class="row">
							...
							function() {
								...
								$("#searchBtn")
										.on(
												"click",
												function(event) {
													self.location = "${path}/board/stock/list${pageMaker.makeQuery(1)}"
															+ "&searchType="
															+ $(
								...
								.on(
										"click",
										function(event){
											self.location = "${path}/board/stock/write"
