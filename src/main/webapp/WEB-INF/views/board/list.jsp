									...
									<table class="table table-bordered">
										<thead>
											<tr>
												...
												<th>제목</th>
												<td><a
													href="${path}/board/read${pageMaker.makeSearch(pageMaker.criteria.page)}&data_no=${board.data_no}">
														${board.title} </a> <span class="badge bg-teal"><i
														class="fas fa-comment"></i> + ${board.reply_cnt}</span></td>
										...
										<ul class="pagination justify-content-center m-0">
											<c:if test="${pageMaker.prev}">
												<li class="page-item"><a class="page-link"
													href="${path}/board/list${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
											</c:if>
											<c:forEach begin="${pageMaker.startPage}"
												...
												<li class="page-item"
													...
													<a class="page-link"
													href="${path}/board/list${pageMaker.makeQuery(idx)}">${idx}</a>
												</li>
											</c:forEach>
											<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
												<li class="page-item"><a class="page-link"
													href="${path}/board/list?${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
											</c:if>
										</ul>
									</nav>
									<div class="row">
							...
							function() {
								...
								$("#searchBtn")
										.on(
												"click",
												function(event) {
													self.location = "${path}/board/list${pageMaker.makeQuery(1)}"
															+ "&searchType="
															+ $(
